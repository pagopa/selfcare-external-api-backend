...
locals {
  ...
  api_v2_domain      = format("api.%s.%s", var.dns_zone_prefix, var.external_domain)
  logo_api_v2_domain = format("%s.%s", var.dns_zone_prefix, var.external_domain)
  apim_base_url   = "${azurerm_api_management_custom_domain.api_v2_custom_domain.gateway[0].host_name}/external"
}

resource "azurerm_api_management_custom_domain" "api_v2_custom_domain" {
  api_management_id = module.apim.id

  gateway {
    host_name = local.api_v2_domain
...
resource "azurerm_api_management_api_version_set" "apim_external_api_onboarding_auto" {
  name                = format("%s-external-api-v2-onboarding-auto", var.env_short)
  resource_group_name = azurerm_resource_group.rg_api.name
  api_management_name = module.apim.name
...
resource "azurerm_api_management_api_version_set" "apim_external_api_onboarding_io" {
  name                = format("%s-external-api-v2-onboarding-io", var.env_short)
  resource_group_name = azurerm_resource_group.rg_api.name
  api_management_name = module.apim.name
...
module "apim_external_api_onboarding_auto_v1" {
  source              = "github.com/pagopa/terraform-azurerm-v3.git//api_management_api?ref=v8.18.0"
  name                = format("%s-external-api-v2-onboarding-auto", local.project)
  api_management_name = module.apim.name
  resource_group_name = azurerm_resource_group.rg_api.name
  ...
  content_value = templatefile("./api/external-api-onboarding-auto/v1/open-api.yml.tpl", {
    host     = azurerm_api_management_custom_domain.api_v2_custom_domain.gateway[0].host_name
    basePath = "/onboarding-api/v1"
  })

  xml_content = templatefile("./api/jwt_base_policy.xml.tpl", {
    API_DOMAIN                 = local.api_v2_domain
  ...
  api_operation_policies = [
    {
      ...
      xml_content = templatefile("./api/ms_external_api/v2/getInstitutions_op_policy.xml.tpl", {
        LOGO_URL                   = "https://${local.logo_api_v2_domain}"
        API_DOMAIN                 = local.api_v2_domain
        KID                        = data.azurerm_key_vault_secret.jwt_kid.value
        JWT_CERTIFICATE_THUMBPRINT = azurerm_api_management_certificate.jwt_certificate.thumbprint
